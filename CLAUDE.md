...
| `Validation/Rules/` | All 70 rule classes, including TOFU's own `RequiredFileRule` (`custom_required_file`), `MaxMbRule` (`max_mb`) and `MimeTypeRule` (`mime_type`). Several labels deliberately share one class (`regex`/`matches`, `integer`/`number`, `default`/`defaults`) — labels are never removed, since an unknown rule name throws at parse time and would white-screen a live form |
